...
public class MakeOrderCommand implements Command {
	 ...
	 * service working with dao layer.
	 */
	private final PrescriptionDaoService prescriptionDaoService
			= new PrescriptionDaoServiceImpl();
	/**
	 * extracts OrderObject from request.
	 */
	private final Extractor<List<OrderDTO>> extractor
			= new OrderDtoListExtractor();
			...
			orderDTOs.forEach(x -> {
				...
				x.setMedicineProductDTO(
						medicineProductDaoService.readById(
								x.getMedicineProductDTO().getId()));
			});
			Set<PrescriptionDTO> userPrescriptions = service
					.readPrescriptions(userDTO.getHealthCareCardNumber());
			if (ValidatorUtil.isValidOrders(userPrescriptions, orderDTOs)) {
				userDTO.getPersonalInfoDTO().setPersonalAccount(
						userDTO.getPersonalInfoDTO().getPersonalAccount()
								.subtract(totalPrice));
				...
				for (PrescriptionDTO prescriptionDTO : userPrescriptions) {
					for (OrderDTO orderDTO : orderDTOs) {
						if (prescriptionDTO.getMedicineProductDTO()
							...
							if (prescriptionDTO.getAmount()
									== orderDTO.getAmount()) {
								prescriptionDaoService.deleteById(
