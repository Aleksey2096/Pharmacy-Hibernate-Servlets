...
public final class PrescriptionRequestDtoExtractor
    ...
    public PrescriptionRequestDTO extract(final HttpServletRequest request) {
        PrescriptionRequestDTO prescriptionRequestDTO
                = PrescriptionRequestDTO.builder()
                .id(RequestDataUtil.getLong(PRESCRIPTION_REQUEST_ID_DB,
                        request))
        ...
        RequestDataUtil.saveFile(
                request.getServletContext().getRealPath(EMPTY_PATH)
                        + prescriptionScanPath, request, IMAGE);
