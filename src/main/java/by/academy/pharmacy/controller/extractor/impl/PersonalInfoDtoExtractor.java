...
public final class PersonalInfoDtoExtractor implements Extractor<PersonalInfoDTO> {
    @Override
    public PersonalInfoDTO extract(final HttpServletRequest request) {
        return PersonalInfoDTO.builder()
                ...
                .personalAccount(RequestDataUtil.getBigDecimal(PERSONAL_ACCOUNT_DB, request))
                .paymentCardNumber(RequestDataUtil.getLong(PAYMENT_CARD_NUMBER_DB, request))
                .build();
    }
}
