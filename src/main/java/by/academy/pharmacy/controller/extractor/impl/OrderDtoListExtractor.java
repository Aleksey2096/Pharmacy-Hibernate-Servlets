...
public final class OrderDtoListExtractor implements Extractor<List<OrderDTO>> {
	...
	public List<OrderDTO> extract(final HttpServletRequest request) {
		Long paymentCardNumber = RequestDataUtil.getLong(PAYMENT_CARD_NUMBER_DB,
				request);
		String contactPhone = RequestDataUtil.getString(CONTACT_PHONE_DB,
				request);
		String deliveryAddress = RequestDataUtil.getString(DELIVERY_ADDRESS_DB,
				request);
